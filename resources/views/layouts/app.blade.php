...
<head>
    ...
    <title>@yield('title', 'Dashboard') | Milky Garage</title>
    <link rel="icon" type="image/webp" href="{{ asset('img/Milky Garage Assets/Logo Area.webp') }}">
    <link rel="shortcut icon" type="image/webp" href="{{ asset('img/Milky Garage Assets/Logo Area.webp') }}">
    <link rel="apple-touch-icon" href="{{ asset('img/Milky Garage Assets/Logo Area.webp') }}">
    <script>
...
<body>
    ...
    <div class="shell">
        <aside class="sidebar">
            <div style="padding: 24px 24px 16px;">
                @auth
                    <div style="display:flex; align-items:flex-start; gap:12px; margin-bottom:16px;">
                        <img src="{{ asset('img/' . auth()->user()->avatarPath()) }}" alt="Avatar" style="width:48px; height:48px; border-radius:12px; object-fit:cover; border:2px solid var(--primary-soft); flex-shrink:0;">
                        <div style="flex-grow:1; min-width:0;">
                            <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:6px;">
                                <div style="font-weight:800; font-size:18px; color:var(--ink); line-height:1.2; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" title="{{ ucwords(auth()->user()->username) }}">{{ ucwords(auth()->user()->username) }}</div>
                            </div>
                            
                            @if(auth()->user()->isRole('owner'))
                                <div class="role-badge" style="margin-top:0; font-size:10px; padding:4px 8px; display:inline-flex;">{{ auth()->user()->roleLabel() }}</div>
                            @else
                                <div class="role-badge" style="margin-top:0; font-size:11px;">{{ auth()->user()->roleLabel() }}</div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="zeropay-card">
                        <div class="zeropay-copy">
                            ...
                            <div id="sidebarZeroPayBalance" class="zeropay-balance" title="Rp {{ number_format(auth()->user()->balance, 0, ',', '.') }}">Rp {{ number_format(auth()->user()->balance, 0, ',', '.') }}</div>
                        </div>
                        <a href="#" class="zeropay-action" data-zeropay-modal-open title="Kelola ZeroPay">@include('components.ui-icon', ['name' => 'bolt', 'class' => 'ui-svg'])</a>
                    </div>
                ...
                @else
                    <div class="brand">
                        <span class="brand-mark">
                            <img src="{{ asset('img/Milky Garage Assets/Logo Area.webp') }}" alt="Logo" style="width:100%; height:100%; object-fit:contain;">
                        </span>
            ...
            @auth
                <nav class="nav">
                    <a class="{{ request()->routeIs('dashboard') ? 'active' : '' }}" href="{{ route('dashboard') }}"><span class="nav-icon nav-icon-dashboard">@include('components.ui-icon', ['name' => 'dashboard', 'class' => 'ui-svg'])</span><span>Dashboard</span></a>
                    @if(auth()->user()->isRole('mechanic'))
                        <a class="{{ request()->routeIs('bookings.*') ? 'active' : '' }}" href="{{ route('bookings.index') }}"><span class="nav-icon nav-icon-booking">@include('components.ui-icon', ['name' => 'booking', 'class' => 'ui-svg'])</span><span>ACC Booking</span></a>
                        <a class="{{ request()->routeIs('service-orders.*') ? 'active' : '' }}" href="{{ route('service-orders.index') }}"><span class="nav-icon nav-icon-service">@include('components.ui-icon', ['name' => 'service', 'class' => 'ui-svg'])</span><span>Laporan Gaji</span></a>
                        <a class="{{ request()->routeIs('reports.analytics') ? 'active' : '' }}" href="{{ route('reports.analytics') }}"><span class="nav-icon nav-icon-agent">@include('components.ui-icon', ['name' => 'agent', 'class' => 'ui-svg'])</span><span>Agent AI</span></a>
                        <a class="{{ request()->routeIs('account.activities') ? 'active' : '' }}" href="{{ route('account.activities') }}"><span class="nav-icon nav-icon-history">@include('components.ui-icon', ['name' => 'history', 'class' => 'ui-svg'])</span><span>Riwayat Akun</span></a>
                    @elseif(auth()->user()->isRole('cashier'))
                        <a class="{{ request()->routeIs('payments.*') ? 'active' : '' }}" href="{{ route('payments.index') }}"><span class="nav-icon nav-icon-payment">@include('components.ui-icon', ['name' => 'payment', 'class' => 'ui-svg'])</span><span>Kelola Pembayaran</span></a>
                        <a class="{{ request()->routeIs('reports.cashier') ? 'active' : '' }}" href="{{ route('reports.cashier') }}"><span class="nav-icon nav-icon-report">@include('components.ui-icon', ['name' => 'report', 'class' => 'ui-svg'])</span><span>Laporan Kasir</span></a>
                        <a class="{{ request()->routeIs('service-orders.*') ? 'active' : '' }}" href="{{ route('service-orders.index') }}"><span class="nav-icon nav-icon-service">@include('components.ui-icon', ['name' => 'service', 'class' => 'ui-svg'])</span><span>Laporan Gaji</span></a>
                        <a class="{{ request()->routeIs('reports.analytics') ? 'active' : '' }}" href="{{ route('reports.analytics') }}"><span class="nav-icon nav-icon-agent">@include('components.ui-icon', ['name' => 'agent', 'class' => 'ui-svg'])</span><span>Agent AI</span></a>
                        <a class="{{ request()->routeIs('account.activities') ? 'active' : '' }}" href="{{ route('account.activities') }}"><span class="nav-icon nav-icon-history">@include('components.ui-icon', ['name' => 'history', 'class' => 'ui-svg'])</span><span>Riwayat Akun</span></a>
                    @elseif(auth()->user()->isRole('customer'))
                        <a class="{{ request()->routeIs('bookings.*') ? 'active' : '' }}" href="{{ route('bookings.index') }}"><span class="nav-icon nav-icon-booking">@include('components.ui-icon', ['name' => 'booking', 'class' => 'ui-svg'])</span><span>Booking Service</span></a>
                        <a class="{{ request()->routeIs('payments.*') ? 'active' : '' }}" href="{{ route('payments.index') }}"><span class="nav-icon nav-icon-payment">@include('components.ui-icon', ['name' => 'payment', 'class' => 'ui-svg'])</span><span>Pembayaran</span></a>
                        <a class="{{ request()->routeIs('reports.analytics') ? 'active' : '' }}" href="{{ route('reports.analytics') }}"><span class="nav-icon nav-icon-agent">@include('components.ui-icon', ['name' => 'agent', 'class' => 'ui-svg'])</span><span>Agent AI</span></a>
                        <a class="{{ request()->routeIs('service-orders.*') ? 'active' : '' }}" href="{{ route('service-orders.index') }}"><span class="nav-icon nav-icon-service">@include('components.ui-icon', ['name' => 'service', 'class' => 'ui-svg'])</span><span>Riwayat Servis</span></a>
                        <a class="{{ request()->routeIs('account.activities') ? 'active' : '' }}" href="{{ route('account.activities') }}"><span class="nav-icon nav-icon-history">@include('components.ui-icon', ['name' => 'history', 'class' => 'ui-svg'])</span><span>Riwayat Akun</span></a>
                    @elseif(auth()->user()->isRole('owner'))
                        <a class="{{ request()->routeIs('reports.finance') ? 'active' : '' }}" href="{{ route('reports.finance') }}"><span class="nav-icon nav-icon-report">@include('components.ui-icon', ['name' => 'report', 'class' => 'ui-svg'])</span><span>Keuangan Harian</span></a>
                        <a class="{{ request()->routeIs('catalog.*') ? 'active' : '' }}" href="{{ route('catalog.index') }}"><span class="nav-icon nav-icon-catalog">@include('components.ui-icon', ['name' => 'catalog', 'class' => 'ui-svg'])</span><span>Master Data</span></a>
                        <a class="{{ request()->routeIs('reports.analytics') ? 'active' : '' }}" href="{{ route('reports.analytics') }}"><span class="nav-icon nav-icon-trend">@include('components.ui-icon', ['name' => 'trend', 'class' => 'ui-svg'])</span><span>Trend Analyze</span></a>
                        <a class="{{ request()->routeIs('account.activities') ? 'active' : '' }}" href="{{ route('account.activities') }}"><span class="nav-icon nav-icon-history">@include('components.ui-icon', ['name' => 'history', 'class' => 'ui-svg'])</span><span>Riwayat Akun</span></a>
                    @endif
                </nav>
            @else
                <nav class="nav">
                    <a class="{{ request()->routeIs('login') ? 'active' : '' }}" href="{{ route('login') }}"><span class="nav-icon nav-icon-login">@include('components.ui-icon', ['name' => 'login', 'class' => 'ui-svg'])</span><span>Login</span></a>
                    <a class="{{ request()->routeIs('register') ? 'active' : '' }}" href="{{ route('register') }}"><span class="nav-icon nav-icon-register">@include('components.ui-icon', ['name' => 'register', 'class' => 'ui-svg'])</span><span>Registrasi</span></a>
                </nav>
            @endauth
        </aside>
        ...
        <main class="main">
            <div class="topbar">
                <div>
                    @php $eyebrow = trim($__env->yieldContent('eyebrow', 'Ai Agent - ZoruAi Artificial Intelligence')); @endphp
                    @if($eyebrow !== '')
                ...
                <div class="page-actions" style="display:flex; gap:10px; align-items:center;">
                    @yield('actions')
                    <a href="{{ route('account.edit') }}" title="Pengaturan" style="display:grid; place-items:center; width:40px; height:40px; border-radius:12px; background:var(--bg); border:1px solid var(--line); color:var(--muted); transition:0.2s;" onmouseover="this.style.color='var(--ink)'; this.style.borderColor='var(--primary)';" onmouseout="this.style.color='var(--muted)'; this.style.borderColor='var(--line)';">
                        @include('components.ui-icon', ['name' => 'settings', 'class' => 'ui-svg'])
                    </a>
                    <form method="POST" action="{{ route('logout') }}" style="margin:0;">
                        ...
                        <button type="submit" title="Logout" style="display:grid; place-items:center; width:40px; height:40px; border-radius:12px; background:var(--danger); border:1px solid var(--danger); color:#fff; cursor:pointer; transition:0.2s;" onmouseover="this.style.opacity='0.8';" onmouseout="this.style.opacity='1';">
                            @include('components.ui-icon', ['name' => 'logout', 'class' => 'ui-svg'])
                        </button>
                    </form>
                </div>
            </div>
    ...
    @if(session('unclaimed_salary_prompt') && auth()->user()->isRole('cashier'))
        <div id="salaryPromptModal" style="position:fixed; top:0; left:0; right:0; bottom:0; background:rgba(0,0,0,0.5); display:flex; align-items:center; justify-content:center; z-index:9999;">
            <div style="background:var(--bg); padding:30px; border-radius:12px; max-width:400px; width:100%; text-align:center; box-shadow:0 10px 30px rgba(0,0,0,0.2);">
                ...
                <p class="muted" style="margin-bottom:20px;">Anda memiliki gaji/fee sebesar <strong>Rp {{ number_format(auth()->user()->unclaimed_salary, 0, ',', '.') }}</strong> hari ini.</p>
                <form method="POST" action="{{ route('cashier.claim-salary') }}" style="display:flex; gap:10px; justify-content:center; margin-bottom:16px;">
                    @csrf
    ...
    @if(auth()->user() && auth()->user()->isRole('customer'))
        @php
            ...
            <script>
                ...
                function clickCustomerNota() {
                    ...
                    if (customerNotaClicks >= 3) {
                        ...
                        window.location.href = "{{ route('messages.read', $unreadNota) }}";
                    }
                }
            </script>
        @endif
        ...
        <div id="zeroPayActionModal" class="zeropay-modal-overlay" aria-hidden="true">
            <section class="zeropay-modal-card" role="dialog" aria-modal="true" aria-labelledby="zeroPayModalTitle">
                <button type="button" class="zeropay-modal-close" data-zeropay-modal-close aria-label="Tutup">
                    @include('components.ui-icon', ['name' => 'close', 'class' => 'ui-svg'])
                </button>
                <div class="zeropay-modal-head">
                    <span class="ui-icon-badge zeropay-modal-icon">
                        @include('components.ui-icon', ['name' => 'bolt', 'class' => 'ui-svg'])
                    </span>
                    <div>
                ...
                <div class="zeropay-modal-actions">
                    <a href="{{ route('topup.index') }}" class="zeropay-action-card">
                        <span class="ui-icon-badge nav-icon-payment">@include('components.ui-icon', ['name' => 'topup', 'class' => 'ui-svg'])</span>
                        <span>Top-up Saldo<small>Tambah saldo ZeroPay</small></span>
                    </a>
                    <a href="{{ route('withdraw.index') }}" class="zeropay-action-card secondary">
                        <span class="ui-icon-badge nav-icon-report">@include('components.ui-icon', ['name' => 'withdraw', 'class' => 'ui-svg'])</span>
    ...
    <div id="qrisOverlay" aria-hidden="true" style="position:fixed; inset:0; background:rgba(0,0,0,0.68); backdrop-filter:blur(10px); display:none; align-items:center; justify-content:center; z-index:99999; padding:18px;">
        <section style="width:min(380px, 100%); border:1px solid color-mix(in srgb, var(--primary) 22%, var(--line)); border-radius:18px; background:color-mix(in srgb, var(--panel-solid) 96%, var(--bg)); color:var(--ink); box-shadow:0 30px 80px rgba(0,0,0,.48); overflow:hidden;">
            <div style="display:flex; gap:14px; align-items:flex-start; padding:24px 26px 18px; border-bottom:1px solid var(--line);">
                <span class="ui-icon-badge zeropay-modal-icon" style="flex-shrink:0;">@include('components.ui-icon', ['name' => 'bolt', 'class' => 'ui-svg'])</span>
                <div>
    ...
    @includeWhen(config('dev_quick_switch.enabled'), 'components.dev-quick-switch')
