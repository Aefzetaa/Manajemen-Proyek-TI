    ...
    <div class="dqs-panel" id="dqsPanel" hidden>
        <div class="dqs-panel-head">
            ...
            <div class="dqs-head-actions">
                <span class="dqs-icon-btn" id="dqsOpenAdd" role="button" tabindex="0" title="Tambah akun" aria-label="Tambah akun">
                    ...
                    <form method="POST" action="{{ route('dev.quick-switch.reset-all') }}" class="dqs-form-reset-all" id="dqsResetForm">
                    @csrf
        ...
        <div class="dqs-body">
            ...
            @foreach($roleOrder as $role)
                ...
                @endphp
                <section class="dqs-group" style="--section-color: {{ $section['color'] }}">
                    <header class="dqs-group-head">
                        <span class="dqs-group-dot" aria-hidden="true"></span>
                        <span class="dqs-group-title">{{ $section['label'] }}</span>
                        <span class="dqs-group-count">{{ $users->count() }}</span>
                    </header>
                    <ul class="dqs-group-list">
                        @foreach($users as $u)
                            @php $isActive = $currentId === $u->id; @endphp
                            <li class="dqs-row-wrap">
                                @if($isActive)
                                    <div class="dqs-row is-active">
                                        <span class="dqs-row-user">{{ $u->username }}</span>
                                        <span class="dqs-row-badge">Aktif</span>
                                    </div>
                                @else
                                    <form method="POST" action="{{ route('dev.quick-switch.switch', $u) }}" class="dqs-form dqs-form-switch">
                                        @csrf
                                ...
                                @endif
                                  <form method="POST" action="{{ route('dev.quick-switch.destroy', $u) }}" class="dqs-form dqs-form-del"
                                      data-confirm="Hapus akun {{ $u->username }}?">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="dqs-icon-btn dqs-icon-del" title="Hapus {{ $u->username }}" aria-label="Hapus {{ $u->username }}">
...
<div class="dqs-modal-overlay" id="dqsAddModal" hidden>
    <div class="dqs-modal-card" role="dialog" aria-labelledby="dqsAddTitle" aria-modal="true">
        ...
        <form method="POST" action="{{ route('dev.quick-switch.store') }}" class="dqs-add-form" id="dqsAddForm">
            @csrf
            <div class="dqs-avatars">
                <x-avatar-picker name="avatar" :size="60" :placeholder="true" :contained="true" />
            </div>
            <div class="dqs-field-row">
                ...
                <label class="dqs-field">
                    <span>Username</span>
                    <input type="text" name="username" value="{{ old('username') }}" required maxlength="8" pattern="[a-zA-Z0-9_]+">
                </label>
            </div>
            <div class="dqs-field-row">
                <label class="dqs-field">
...
{{-- Modal verifikasi reset Dev Quick Switch --}}
...
{{-- Modal kode cheat Dev Quick Switch --}}
