...
<?php $__env->startSection('eyebrow', 'Kelola Layanan dan Suku Cadang'); ?>
...
<div class="catalog-workspace">
                    ...
                    <tbody>
                        <?php $__empty_1 = true; $__currentLoopData = $serviceTypes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $type): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr style="transition:0.2s;">
                                <td style="font-weight:600; color:var(--ink);"><?php echo e($type->name); ?></td>
                                <td><span style="background:var(--bg); padding:4px 8px; border-radius:6px; font-size:12px; border:1px solid var(--line);"><?php echo e($type->estimated_minutes); ?> mnt</span></td>
                                <td style="color:var(--primary); font-weight:700;">Rp <?php echo e(number_format($type->base_price, 0, ',', '.')); ?></td>
                                <td style="text-align:center;">
                                    <div style="display:flex; gap:8px; justify-content:center; flex-wrap:wrap;">
                                        <button class="button small secondary" style="padding:6px 12px; border-radius:8px;" type="button" onclick="document.getElementById('edit-service-<?php echo e($type->id); ?>').style.display = document.getElementById('edit-service-<?php echo e($type->id); ?>').style.display === 'none' ? 'table-row' : 'none'">Edit</button>
                                        <form method="POST" action="<?php echo e(route('catalog.service-types.destroy', $type)); ?>" data-confirm="Yakin ingin menghapus layanan ini secara permanen?" style="margin:0;">
                            ...
                            <tr id="edit-service-<?php echo e($type->id); ?>" style="display:none; background:var(--bg);">
                                <td colspan="4" style="padding:16px;">
                                    <div style="background:var(--panel); padding:16px; border-radius:12px; border:1px solid var(--line);">
                                        <form method="POST" action="<?php echo e(route('catalog.service-types.update', $type)); ?>" style="display:grid; gap:12px;">
                                            <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                                            <div class="grid grid-2" style="gap:12px;">
                                                <input type="text" name="name" value="<?php echo e($type->name); ?>" required placeholder="Nama Layanan">
                                                <input type="number" name="estimated_minutes" value="<?php echo e($type->estimated_minutes); ?>" required placeholder="Estimasi Menit">
                                                <input type="number" name="base_price" value="<?php echo e($type->base_price); ?>" required placeholder="Harga Dasar (Rp)">
                                                <input type="number" name="mechanic_salary" min="0" max="100" value="<?php echo e($type->mechanicSharePercent()); ?>" required placeholder="Persentase Mekanik (%)">
                                                <input type="number" name="cashier_salary" min="0" max="100" value="<?php echo e($type->cashierSharePercent()); ?>" required placeholder="Persentase Kasir (%)">
                                            </div>
                                            <div style="display:flex; justify-content:flex-end; gap:8px;">
                                                <button class="button small" type="submit">Simpan Perubahan</button>
                                                <button type="button" class="button small secondary" onclick="document.getElementById('edit-service-<?php echo e($type->id); ?>').style.display='none'">Batal</button>
                                            </div>
                                        </form>
                                    </div>
                                </td>
                ...
                <table>
                    ...
                    <tbody>
                        <?php $__empty_1 = true; $__currentLoopData = $promotions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $promo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr>
                                <td>
                                    <div style="font-weight:700; color:var(--ink);"><?php echo e($promo->title); ?></div>
                                    <div style="font-size:11px; color:var(--muted); margin-top:2px;"><?php echo e(\Illuminate\Support\Str::limit($promo->description, 80)); ?></div>
                                    <div style="font-size:11px; color:var(--muted); margin-top:4px;">
                                        <?php echo e($promo->starts_at?->format('d/m/Y H:i') ?? 'Mulai sekarang'); ?> - <?php echo e($promo->ends_at?->format('d/m/Y H:i') ?? 'Tanpa batas'); ?>

                                    </div>
                                </td>
                                <td><?php echo e($promo->serviceType?->name ?? 'Semua layanan'); ?></td>
                                <td style="color:var(--accent); font-weight:800;"><?php echo e($promo->discount_percent); ?>%</td>
                                <td>
                                    <span class="status <?php echo e($promo->is_active ? 'paid' : 'canceled'); ?>"><?php echo e($promo->is_active ? 'Aktif' : 'Nonaktif'); ?></span>
                                </td>
                                <td style="text-align:center;">
                                    <div style="display:flex; gap:8px; justify-content:center; flex-wrap:wrap;">
                                        <button class="button small secondary" style="padding:6px 12px; border-radius:8px;" type="button" onclick="document.getElementById('edit-promo-<?php echo e($promo->id); ?>').style.display = document.getElementById('edit-promo-<?php echo e($promo->id); ?>').style.display === 'none' ? 'table-row' : 'none'">Edit</button>
                                        <form method="POST" action="<?php echo e(route('catalog.promotions.destroy', $promo)); ?>" data-confirm="Yakin ingin menghapus promo ini?" style="margin:0;">
                                            <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                            ...
                            <tr id="edit-promo-<?php echo e($promo->id); ?>" style="display:none; background:var(--bg);">
                                <td colspan="5" style="padding:16px;">
                                    <div style="background:var(--panel); padding:16px; border-radius:12px; border:1px solid var(--line);">
                                        <form method="POST" action="<?php echo e(route('catalog.promotions.update', $promo)); ?>" style="display:grid; gap:12px;">
                                            <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                                            <div class="grid grid-2" style="gap:12px;">
                                                <input type="text" name="title" value="<?php echo e($promo->title); ?>" required placeholder="Judul Promo">
                                                <select name="service_type_id">
                                                    ...
                                                    <?php $__currentLoopData = $serviceTypes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $type): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                        <option value="<?php echo e($type->id); ?>" <?php if($promo->service_type_id === $type->id): echo 'selected'; endif; ?>><?php echo e($type->name); ?></option>
                                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                                </select>
                                                <input type="number" name="discount_percent" min="1" max="100" value="<?php echo e($promo->discount_percent); ?>" required placeholder="Diskon (%)">
                                                <label style="display:flex; align-items:center; gap:8px; font-weight:700;">
                                                    <input type="checkbox" name="is_active" value="1" <?php if($promo->is_active): echo 'checked'; endif; ?> style="width:auto;"> Aktif
                                                </label>
                                                <input type="datetime-local" name="starts_at" value="<?php echo e($promo->starts_at?->format('Y-m-d\TH:i')); ?>">
                                                <input type="datetime-local" name="ends_at" value="<?php echo e($promo->ends_at?->format('Y-m-d\TH:i')); ?>">
                                            </div>
                                            <textarea name="description" rows="3" placeholder="Deskripsi promo"><?php echo e($promo->description); ?></textarea>
                                            <div style="display:flex; justify-content:flex-end; gap:8px;">
                                                <button class="button small" type="submit">Simpan Promo</button>
                                                <button type="button" class="button small secondary" onclick="document.getElementById('edit-promo-<?php echo e($promo->id); ?>').style.display='none'">Batal</button>
                                            </div>
                                        </form>
                                    </div>
                                </td>
                ...
                <table>
                    ...
                    <tbody>
                        <?php $__empty_1 = true; $__currentLoopData = $spareParts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $part): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr style="transition:0.2s;">
                                <td>
                                    <div style="font-weight:600; color:var(--ink);"><?php echo e($part->name); ?></div>
                                    <div style="font-size:11px; color:var(--muted); font-family:monospace; margin-top:2px;"><?php echo e($part->sku); ?></div>
                                </td>
                                <td>
                                    <?php if($part->stock <= 5): ?>
                                        <span style="background:var(--danger); color:#fff; padding:4px 8px; border-radius:6px; font-size:12px; font-weight:800; border:1px solid var(--danger);"><?php echo e($part->stock); ?></span>
                                    <?php else: ?>
                                        <span style="background:var(--bg); padding:4px 8px; border-radius:6px; font-size:12px; font-weight:700; border:1px solid var(--line);"><?php echo e($part->stock); ?></span>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <span class="dynamic-rng-price" style="font-family:monospace; font-weight:700; color:var(--accent);">Rp <?php echo e(number_format(rand(10000, 50000), 0, ',', '.')); ?></span>
                                </td>
                                <td style="color:var(--primary); font-weight:700;">Rp <?php echo e(number_format($part->price, 0, ',', '.')); ?></td>
                                <td style="text-align:center;">
                                    <div style="display:flex; gap:8px; justify-content:center; flex-wrap:wrap;">
                                        <button class="button small secondary" style="padding:6px 12px; border-radius:8px;" type="button" onclick="document.getElementById('edit-part-<?php echo e($part->id); ?>').style.display = document.getElementById('edit-part-<?php echo e($part->id); ?>').style.display === 'none' ? 'table-row' : 'none'">Edit</button>
                                        <button class="button small secondary" style="padding:6px 12px; border-radius:8px;" type="button" onclick="showRestockForm(<?php echo e($part->id); ?>, '<?php echo e($part->name); ?>')">+ Restock</button>
                                        <form method="POST" action="<?php echo e(route('catalog.spare-parts.destroy', $part)); ?>" data-confirm="Yakin ingin menghapus sparepart ini?" style="margin:0;">
                                            <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                                            <button class="button small" style="background:var(--danger); border-color:var(--danger); color:#fff; padding:6px; border-radius:8px;" type="submit" title="Hapus">
                                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>
                                            </button>
                            ...
                            <tr id="edit-part-<?php echo e($part->id); ?>" style="display:none; background:var(--bg);">
                                <td colspan="5" style="padding:16px;">
                                    <div style="background:var(--panel); padding:16px; border-radius:12px; border:1px solid var(--line);">
                                        <form method="POST" action="<?php echo e(route('catalog.spare-parts.update', $part)); ?>" style="display:grid; gap:12px;">
                                            <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                                            <div class="grid grid-2" style="gap:12px;">
                                                <input type="text" name="name" value="<?php echo e($part->name); ?>" required placeholder="Nama Sparepart">
                                                <input type="text" name="sku" value="<?php echo e($part->sku); ?>" required placeholder="SKU">
                                                <input type="number" name="stock" value="<?php echo e($part->stock); ?>" required placeholder="Stok">
                                                <input type="number" name="price" value="<?php echo e($part->price); ?>" required placeholder="Harga Jual (Rp)">
                                            </div>
                                            <div style="display:flex; justify-content:flex-end; gap:8px;">
                                                <button class="button small" type="submit">Simpan Perubahan</button>
                                                <button type="button" class="button small secondary" onclick="document.getElementById('edit-part-<?php echo e($part->id); ?>').style.display='none'">Batal</button>
                                            </div>
                                        </form>
                                    </div>
                                </td>
                            </tr>
                            <tr id="restock-row-<?php echo e($part->id); ?>" style="display:none; background:var(--bg);">
                                <td colspan="5" style="padding:16px;">
                                    <div style="background:var(--panel); padding:16px; border-radius:12px; border:1px dashed var(--accent);">
                                        <form method="POST" action="<?php echo e(route('catalog.spare-parts.restock', $part)); ?>" onsubmit="return handleQrisSubmit(event, this);" style="margin:0; display:flex; align-items:flex-end; gap:16px; flex-wrap:wrap;">
                                            <?php echo csrf_field(); ?>
                                            <div style="flex:1; min-width:150px;">
                                                <label style="font-size:12px; color:var(--muted); font-weight:600; margin-bottom:4px; display:block;">Restock <strong><?php echo e($part->name); ?></strong></label>
                                            ...
                                            <div style="display:flex; gap:8px;">
                                                ...
                                                <button class="button secondary" style="padding:10px 20px; border-radius:8px;" type="button" onclick="document.getElementById('restock-row-<?php echo e($part->id); ?>').style.display='none'">Batal</button>
                                            </div>
                                        </form>
                                    </div>
                                </td>
    ...
    <div class="stack" style="gap:24px;">
        ...
        <section class="panel catalog-section catalog-form" data-catalog-panel="service" style="background:var(--panel); border:1px solid var(--line); box-shadow:var(--shadow-sm);">
            ...
            <form method="POST" action="<?php echo e(route('catalog.service-types.store')); ?>" class="stack">
                <?php echo csrf_field(); ?>
                <div class="field">
        ...
        <section class="panel catalog-section catalog-form is-active" data-catalog-panel="promo" style="background:var(--panel); border:1px solid var(--line); box-shadow:var(--shadow-sm);">
            ...
            <form method="POST" action="<?php echo e(route('catalog.promotions.store')); ?>" class="stack">
                <?php echo csrf_field(); ?>
                <div class="field">
        ...
        <section class="panel catalog-section catalog-form" data-catalog-panel="sparepart" style="background:var(--panel); border:1px solid var(--line); box-shadow:var(--shadow-sm);">
            ...
            <form method="POST" action="<?php echo e(route('catalog.spare-parts.store')); ?>" class="stack" onsubmit="return handleQrisSubmit(event, this);">
                <?php echo csrf_field(); ?>
                <div class="field">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\ProyekTI\resources\views/catalog/index.blade.php ENDPATH**/ ?>