<?php $__env->startSection('title', 'ZoruAi'); ?>
<?php $__env->startSection('eyebrow', 'Asisten Milky Garage'); ?>
...
<div class="zoru-container">
    ...
    <aside class="zoru-sidebar">
        ...
        <div class="zoru-card">
            <div class="user-avatar-ring">
                <span class="user-avatar-initials"><?php echo e($initials); ?></span>
                <span class="user-role-badge" style="background:var(--primary); color:#fff; font-size:9px; font-weight:800; padding:2px 8px; border-radius:10px; text-transform:uppercase; border:1.5px solid var(--panel-solid); position:absolute; bottom:-4px; white-space:nowrap;"><?php echo e(auth()->user()->roleLabel()); ?></span>
            </div>
            <div class="user-details">
                <h3 class="user-name"><?php echo e(auth()->user()->username); ?></h3>
                <span class="user-role" style="font-size: 11px; color: var(--muted); font-weight: 500; word-break: break-all;"><?php echo e(auth()->user()->email); ?></span>
            </div>
        </div>
    ...
    <main class="zoru-main">
        ...
        <div id="chatBox" class="zoru-chatbox">
            <div class="zoru-bubble zoru-bubble-ai" id="zoruWelcomeBubble"><strong>Halo <?php echo e(auth()->user()->username); ?>! </strong>
        ...
        <?php if(auth()->user()->isRole('owner')): ?>
            ...
            <div id="trendsBox" class="zoru-trendsbox" style="display:none;">
                ...
                <div class="panel zoru-trends-filter">
                    <form method="GET" action="<?php echo e(route('reports.analytics')); ?>" id="trendsFilterForm" class="toolbar" style="justify-content:space-between;">
                        ...
                        <div style="display:flex; align-items:center; gap:14px; flex-wrap:wrap;">
                            ...
                            <div style="display:flex; align-items:center; gap:10px;">
                                ...
                                <select name="quarter" id="quarter" onchange="document.getElementById('trendsFilterForm').submit()" style="max-width:230px;">
                                    <?php $__currentLoopData = $quarterOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $quarterValue => $quarter): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($quarterValue); ?>" <?php if((int) $selectedQuarter === (int) $quarterValue): echo 'selected'; endif; ?>>
                                            <?php echo e($quarter['label']); ?> (<?php echo e($quarter['range']); ?>)
                                        </option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
                            </div>
                        </div>
                        <?php if(request('service_type_id') || (int) request('quarter', 1) !== 1): ?>
                            <a class="button secondary small" href="<?php echo e(route('reports.analytics', ['tab' => 'trends'])); ?>">Reset Filter</a>
                        <?php endif; ?>
                    </form>
                </div>
                ...
                <div class="panel zoru-trends-chart-panel">
                    <h3 style="margin-top:0; font-size:18px; font-weight:800; border-bottom:1px solid var(--line); padding-bottom:12px; margin-bottom:20px; display:flex; align-items:center; gap:8px;">
                        ...
                        <span style="font-size:13px; font-weight:600; color:var(--primary-dark); background:var(--primary-soft); padding:3px 10px; border-radius:20px;">
                            <?php echo e($trendYear); ?>

                        </span>
                        <?php if(request('service_type_id') && $serviceTypes->firstWhere('id', request('service_type_id'))): ?>
                            <span style="font-size:13px; font-weight:600; color:var(--primary-dark); background:var(--primary-soft); padding:3px 10px; border-radius:20px;">
                                <?php echo e($serviceTypes->firstWhere('id', request('service_type_id'))->name); ?>
                        ...
                        <?php $__currentLoopData = $monthlyTrends; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $trend): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            <div style="display:flex; flex-direction:column; align-items:center; gap:12px; height:100%; justify-content:flex-end;">
                                ...
                                <div style="background:var(--bg-soft); border:1px solid var(--line); border-radius:6px; padding:4px 10px; font-size:13px; font-weight:800; color:var(--primary-dark); box-shadow:var(--shadow-soft);">
                                    <?php echo e($trend['count']); ?> Servis
                                </div>
                                
                                <!-- Bar -->
                                <div style="width:100%; max-width:60px; height:<?php echo e($percent); ?>%; min-height:8px; background:linear-gradient(180deg, var(--primary), var(--primary-soft)); border-radius:8px 8px 0 0; transition:height 0.4s ease; box-shadow:0 0 15px var(--primary-soft); position:relative; overflow:hidden;">
                                    <!-- Shimmer effect -->
                    ...
                    <p class="muted" style="font-size:12px; margin:0; text-align:right;">* Menampilkan statistik data penyelesaian servis lunas periode <?php echo e($quarterOptions[$selectedQuarter]['range']); ?> <?php echo e($trendYear); ?>.</p>
                </div>
            </div>
        <?php endif; ?>
...
<script>
    ...
    const servicesList = <?php echo json_encode($allServices, 15, 512) ?>;
    const sparePartsList = <?php echo json_encode($allSpareParts, 15, 512) ?>;
    const userVehiclesList = <?php echo json_encode($userVehicles, 15, 512) ?>;
    ...
    function submitZoruPrompt(promptText) {
        ...
        fetch("<?php echo e(route('reports.analytics.zoru')); ?>", {
            method: 'POST',
            headers: {
    ...
    .main { min-height: 0; overflow: hidden; display: flex; flex-direction: column; }
                    ...
                    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; margin-bottom: 16px;">
                        <a href="<?php echo e(route('reports.finance')); ?>" style="display: flex; flex-direction: column; gap: 6px; background: rgba(255,255,255,0.02); border: 1px solid var(--line); border-radius: 10px; padding: 12px; text-decoration: none; transition: all 0.2s;" class="owner-gate-card">
                        ...
                        <a href="<?php echo e(route('catalog.index')); ?>" style="display: flex; flex-direction: column; gap: 6px; background: rgba(255,255,255,0.02); border: 1px solid var(--line); border-radius: 10px; padding: 12px; text-decoration: none; transition: all 0.2s;" class="owner-gate-card">
                            <span style="font-size: 18px;"></span>
    ...
    function payRestock(token, btn) {
        ...
        const submitRestockPayment = () => {
            fetch("<?php echo e(route('reports.analytics.zoru.restock')); ?>", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
    ...
    function cancelRestock(token, btn) {
        ...
        fetch("<?php echo e(route('reports.analytics.zoru.restock.cancel')); ?>", {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\ProyekTI\resources\views/reports/analytics.blade.php ENDPATH**/ ?>